...
import { SelectedDatesList } from "./SelectedDatesS";
    ...
    if (date && time) {
      ...
      try {
        const docRef = await addDoc(collection(db, "selectedDates"), {
          email: user.email, // Store the user's email instead of userId
          dateTime: formattedDateTime,
          timestamp: new Date(),
        });
  
        setSelectedDates((prev) => [...prev, { id: docRef.id, dateTime: formattedDateTime, email: user.email }]);
        setDate(undefined);
        setTime(undefined);
      } catch (error) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="mt-4">
        ...
        <SelectedDatesList dates={selectedDates} onComplete={handleComplete} onDelete={handleDelete} />
      </div>
    </div>
  );
}
